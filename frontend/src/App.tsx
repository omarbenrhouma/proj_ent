...
function ProfileView({ profile, onSaved, onBack, onLogout }: { profile: CandidateProfile; onSaved: (profile: CandidateProfile) => void; onBack: () => void; onLogout: () => void }) {
  ...
  const [error, setError] = useState('')
  async function save(event: React.FormEvent) { event.preventDefault(); setBusy(true); setError(''); setMessage(''); try { onSaved(await api.updateProfile({ phone, education, experience })); setMessage('Profil enregistré.') } catch (reason) { setError((reason as Error).message) } finally { setBusy(false) } }
  ...
  return <><Header onLogout={onLogout} /><main className="page profile-page"><button className="quiet-button back-button" onClick={onBack}>← Retour au parcours</button><div className="profile-heading"><p className="eyebrow">ESPACE CANDIDAT</p><h1>Votre profil.</h1><p className="muted">Ces informations accompagnent votre candidature et restent modifiables avant la soumission.</p></div><div className="profile-grid"><section className="panel profile-card"><div className="profile-identity"><span className="profile-avatar">{profile.first_name[0]}{profile.last_name[0]}</span><div><h2>{profile.first_name} {profile.last_name}</h2><p>{profile.email}</p></div></div><form onSubmit={save}><label>Téléphone<input value={phone} onChange={(event) => setPhone(event.target.value)} placeholder="+33 6 00 00 00 00" /></label><label>Formation<textarea value={education} onChange={(event) => setEducation(event.target.value)} rows={4} placeholder="Votre parcours de formation" /></label><label>Expérience<textarea value={experience} onChange={(event) => setExperience(event.target.value)} rows={5} placeholder="Vos expériences principales" /></label>{error && <p className="error">{error}</p>}{message && <p className="notice">{message}</p>}<button className="primary-button" disabled={busy}>{busy ? 'Enregistrement...' : 'Enregistrer le profil'}</button></form></section><aside className="panel cv-card"><p className="eyebrow">DOCUMENT CANDIDATURE</p><h2>Votre CV</h2><p className="muted">PDF, DOC ou DOCX, 5 Mo maximum.</p><div className="cv-file"><span className="cv-icon">↗</span><span>{profile.cv_name ?? 'Aucun CV ajouté'}</span></div><label className="upload-button">{busy ? 'Traitement...' : 'Ajouter ou remplacer le CV'}<input type="file" accept=".pdf,.doc,.docx,application/pdf" onChange={upload} disabled={busy} /></label></aside></div></main></>
}
