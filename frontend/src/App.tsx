...
function App() {
  ...
  if (view === 'result' && score) return <ResultView onLogout={logout} />
...
function Auth({ onAuthenticated }: { onAuthenticated: (token: string) => void }) {
  ...
  return <main className="auth-shell"><section className="auth-hero"><div className="auth-hero-pattern" aria-hidden="true">{Array.from({ length: 60 }, (_, item) => <span key={item} />)}</div><div className="auth-hero-copy"><div className="brand"><span className="brand-mark">Q</span><span>Quorum</span></div><h1>Montrez ce que vous savez faire.</h1><p>Une évaluation claire, équitable et conçue pour révéler vos compétences.</p></div></section><form className="auth-card" onSubmit={submit}><span className="section-title">Espace candidat</span><h2>{registerMode ? 'Créer votre compte' : 'Bienvenue'}</h2>{registerMode && <div className="form-row"><label>Prénom<input value={firstName} onChange={(event) => setFirstName(event.target.value)} required /></label><label>Nom<input value={lastName} onChange={(event) => setLastName(event.target.value)} required /></label></div>}<label>Email<input type="email" value={email} onChange={(event) => setEmail(event.target.value)} required /></label><label>Mot de passe<input type="password" value={password} onChange={(event) => setPassword(event.target.value)} minLength={8} required /></label>{error && <p className="error">{error}</p>}<button className="primary-button" disabled={busy}>{busy ? 'Connexion...' : registerMode ? 'Créer le compte' : 'Se connecter'}</button><button type="button" className="link-button" onClick={() => setRegisterMode(!registerMode)}>{registerMode ? 'J’ai déjà un compte' : 'Créer un compte candidat'}</button></form></main>
}
...
function ResultView({ onLogout }: { onLogout: () => void }) { return <><Header onLogout={onLogout} /><main className="page result-page"><section className="thank-you-card"><h1>Merci pour votre participation.</h1><p className="muted">Vos réponses ont bien été enregistrées. Notre équipe reviendra vers vous prochainement.</p></section></main></> }
