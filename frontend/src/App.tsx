...
function TestView({ questions, question, index, total, answer, remaining, onAnswer, onJump, onPrevious, onNext, onSubmit, busy, error }: { questions: Question[]; question: Question; index: number; total: number; answer: string; remaining: number | null; onAnswer: (value: string) => void; onJump: (index: number) => void; onPrevious: () => void; onNext: () => void; onSubmit: () => void; busy: boolean; error: string }) {
  ...
  const [openCategory, setOpenCategory] = useState(question.category)
  useEffect(() => { setOpenCategory(question.category) }, [question.category])
  return <main className="test-shell"><header className="test-topbar"><div className="brand"><span className="brand-mark">Q</span><span>Quorum</span></div><div className={`timer${low ? ' timer-low' : ''}`}><span className={`timer-dot${low ? ' pulse' : ''}`} /> <span className="num">{minutes}:{seconds}</span></div></header><div className="test-layout"><aside className="question-nav">{groups.map(([category, items]) => { const open = category === openCategory; const answered = items.filter((item) => item < index).length; return <div className={`nav-group ${open ? 'open' : ''}`} key={category}><button type="button" className="nav-group-title" onClick={() => setOpenCategory(open ? '' : category)}><span>{category}</span><span className="num nav-group-count">{answered}/{items.length}</span></button>{open && <div className="nav-grid">{items.map((item) => <button key={item} className={`question-chip ${item === index ? 'active' : ''} ${item < index ? 'answered' : ''}`} onClick={() => onJump(item)}>{(item + 1).toString().padStart(2, '0')}</button>)}</div>}</div> })}</aside><section className="question-panel"><div className="question-heading"><span className="num">{index + 1} / {total}</span><span className="tag">{question.category}</span></div><div className="progress">{Array.from({ length: total }, (_, item) => <span key={item} className={item <= index ? 'tick filled' : 'tick'} />)}</div><article key={question.id} className="question-enter"><div className="question-badges"><span className="tag-muted">Choix unique</span><span className="tag-muted num">{question.points} pts</span></div><h1>{question.statement}</h1><div className="options">{question.options.map((option) => <label className={`option ${answer === option.key ? 'selected' : ''}`} key={option.key}><input type="radio" name={question.id} checked={answer === option.key} onChange={() => onAnswer(option.key)} /><span className="option-key">{option.key}</span><span>{option.label}</span></label>)}</div></article>{error && <p className="error">{error}</p>}<footer className="question-actions"><button className="quiet-button" onClick={onPrevious} disabled={index === 0}>Précédente</button>{index === total - 1 ? <button className="primary-button" onClick={onSubmit} disabled={busy}>Soumettre le test</button> : <button className="primary-button" onClick={onNext}>Question suivante</button>}</footer></section></div></main>
}
