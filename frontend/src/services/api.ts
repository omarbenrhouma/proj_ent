...
export type CandidateDetail = CandidateRow & { phone: string | null; education: string | null; experience: string | null; breakdown: Record<string, unknown>; answers: CandidateAnswerDetail[] }
...
export type CandidateProfile = { user_id: string; first_name: string; last_name: string; email: string; phone: string | null; education: string | null; experience: string | null; cv_name: string | null }
...
export const api = {
  ...
  state: (campaignId: string) => request<TestState>(`/api/v1/candidate/test?campaign_id=${campaignId}`),
  start: (campaignId: string) => request<TestState>(`/api/v1/candidate/test/start?campaign_id=${campaignId}`, { method: 'POST' }),
  questions: (campaignId: string) => request<Question[]>(`/api/v1/candidate/test/questions?campaign_id=${campaignId}`),
  savedAnswers: (campaignId: string) => request<Record<string, { answer: string; marked_for_review: boolean }>>(`/api/v1/candidate/test/answers?campaign_id=${campaignId}`),
  ...
  updateProfile: (payload: Pick<CandidateProfile, 'phone' | 'education' | 'experience'>) => request<CandidateProfile>('/api/v1/candidate/profile', { method: 'PUT', body: JSON.stringify(payload) }),
